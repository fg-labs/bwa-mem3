...
near-linearly with thread count (serial densify of hg38 is ~11 min; 16-way is
~40 s). Meaningful only with `-u`; a value `> 1` without `-u` warns and is
ignored.
...
**`-u 2` is the best speed-per-GB operating point and the recommended default.**
...
The gain is byte-identical either way (denser sampling changes only LF-walk step
counts, never the resolved coordinate) and scales with how resolve-heavy the
workload is and how large the reference is, so treat the percentages as
indicative rather than a guarantee for your data.
