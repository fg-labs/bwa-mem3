...
thread pool sized by `--bam-threads`, which defaults to `-t/8`, so serial
compression is no longer the throughput bottleneck by default. Output stays
byte-identical to the single-threaded path. Pass `--bam-threads 0` to force the
