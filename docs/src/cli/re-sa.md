...
just report the index's current SA rate.
...
`re-sa` writes it to disk once, so every later `mem` (and every `shm` stage)
picks up the new rate with no extra flags.
...
```bash
cp ref.fa.bwt.2bit.64 ref.fa.pac ref.fa.ann ref.fa.amb  /idx/dense/
...
- `INT` **smaller** than the current on-disk shift **densifies** the table. Each
  ...
  [`index -u INT`](index-cmd.md#-u-int--sa-sample-rate).
...
near-linearly with thread count. It has **no effect when coarsening** (a
memory-bandwidth-bound decimation, run serially) or **when inspecting** (no
`-u`); passing `-t` in those cases warns and is ignored.

## Performance

A denser on-disk table gives the same alignment speed-up as staging one with
[`shm -u`](shm.md#-u-int--densify-the-staged-sa-sample-table), but permanently
and with no per-run staging cost. Representative figures on hg38, a
5M-read-pair WGS slice (Graviton4, arm64/NEON tier, 16 threads), as relative
user-CPU deltas versus a plain stride-8 index (indicative, not absolute):
...
> from the original, and from a freshly built index at the same rate.
>
...
> the sibling index files (`.pac`, `.ann`, `.amb`, and any `.0123`) are left
> alone. Because those siblings are required for alignment, `re-sa` rewrites the
> index **in place** rather than to a new prefix — a new prefix would orphan
> them. To keep a copy at a different rate, copy the whole index first, then
> `re-sa` the copy.
>
> **Note — the write is atomic**
>
> The new table is written to a `.tmp` sibling and `rename(2)`d over the
> original on success, so an interrupted run never leaves a partial or corrupt
> index at the canonical path — a concurrent reader sees either the old index or
> the new one.
>
> **Warning — drop any staged shared-memory segment first**
>
> Shared memory has no staleness check. If the index is currently staged
> (`bwa-mem3 shm <idxbase>`), a stale segment will keep serving the old SA rate
> and can silently mis-align after the on-disk file changes. `re-sa` detects this
> and prints a warning when it rewrites a staged index, but it still proceeds —
> run `bwa-mem3 shm -d` before re-aligning, then re-stage if desired.
