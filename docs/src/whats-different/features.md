...
| Contained-seed extension skip (`--keep-contained-ext` opts out) | [#192](https://github.com/fg-labs/bwa-mem3/pull/192) | — | fork-only (**on by default**; byte-identical to the reference extension path on all read lengths, `--meth` included; `--compat` and `--keep-contained-ext` run the reference path; `--skip-contained-ext` is deprecated and accepted as a no-op) |
