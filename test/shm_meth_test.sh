...
"$BIN" shm --meth "$PLAIN_PREFIX" > /dev/null 2>&1 \
    || {
        echo "FAIL A: shm --meth <plain> exited non-zero" >&2
        exit 1
    }
...
"$BIN" mem --meth "$PLAIN_PREFIX" test/fixtures/reads.fa > /dev/null 2> "$ERR" \
    || {
        echo "FAIL B: mem --meth exited non-zero" >&2
        cat "$ERR" >&2
        exit 1
    }
...
"$BIN" shm --meth "$METH_PREFIX" > /dev/null 2>&1 \
    || {
        echo "FAIL C: shm --meth on already-.meth prefix exited non-zero" >&2
        exit 1
    }
LIST="$("$BIN" shm -l 2>&1)"
...
"$BIN" shm "$PLAIN_PREFIX" > /dev/null 2>&1 \
    || {
        echo "FAIL D: shm <plain> on a dual index unexpectedly failed" >&2
        exit 1
    }
LIST="$("$BIN" shm -l 2>&1)"
