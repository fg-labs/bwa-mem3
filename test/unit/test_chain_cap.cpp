...
// A chain with an explicit weight. mem_chain_cap_extend uses a[i].w directly when > 0
// (no seed access), and drops only free seeds when m > SEEDS_PER_CHAIN — m==0 here, so
// no allocation is needed.
mem_chain_t chain(uint32_t w) {
    mem_chain_t c;
    memset(&c, 0, sizeof(c));
    c.w = w;
    return c;
...
struct CapResult {
    int n;
    int capped_w;
    std::vector<uint32_t> weights;  // surviving weights (kept in input order), sorted for comparison
};
...
CapResult run_cap(std::vector<mem_chain_t> chains, int max_n, float tie_frac, int tie_floor) {
    ...
    r.n = n;
    r.capped_w = capped_w;
    for (int i = 0; i < n; i++) r.weights.push_back(chains[i].w);
    std::sort(r.weights.begin(), r.weights.end());
    return r;
}
...
TEST_CASE("cap: the count cap keeps the top-N by weight and reports max dropped weight") {
    ...
    CHECK(r.n == 2);
    CHECK(r.weights == std::vector<uint32_t>{50, 100});
    CHECK(r.capped_w == 30);
}
...
TEST_CASE("cap: the competitiveness gate prunes non-competitive chains with no count cap") {
    ...
    CHECK(r.n == 2);
    CHECK(r.weights == std::vector<uint32_t>{95, 100});
...
TEST_CASE("cap: tie_floor keeps exactly the top-N; lower-ranked chains still face the gate") {
    ...
    CHECK(kept2.n == 2);
    CHECK(kept2.weights == std::vector<uint32_t>{80, 100});
