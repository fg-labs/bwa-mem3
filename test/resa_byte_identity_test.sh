...
#   H  permission preservation: re-sa keeps the destination index's existing
#      mode (a private 0600 index stays private, not widened to 0644)
#
...
[[ -s "$ref_src" ]] || {
    echo "FAIL: missing fixture $ref_src" >&2
    exit 1
}
[[ -s "$reads" ]] || {
    echo "FAIL: missing fixture $reads" >&2
    exit 1
}
...
fail() {
    echo "FAIL [re-sa byte-identity]: $*" >&2
    exit 1
}
...
"$bin" re-sa -u 2 "$tmp/a_re/ref.fa" > "$tmp/a_re/resa.log" 2>&1 \
    || {
        cat "$tmp/a_re/resa.log" >&2
        fail "case A: re-sa -u 2 failed"
    }
...
"$bin" re-sa -u 4 "$tmp/b_re/ref.fa" > "$tmp/b_re/resa.log" 2>&1 \
    || {
        cat "$tmp/b_re/resa.log" >&2
        fail "case B: re-sa -u 4 failed"
    }
...
build_index "$tmp/e_orig" > /dev/null # stride 8
build_index "$tmp/e_dense" > /dev/null
...
# path, which necessarily differs between the two index dirs. Route each
# pipeline through `fail` so a mem failure (or an empty record set) surfaces the
# marker instead of set -e exiting silently under pipefail.
"$bin" mem "$tmp/e_orig/ref.fa" "$reads" 2> /dev/null | grep -v '^@' > "$tmp/e_orig.sam" \
    || fail "case E: alignment against the original index failed"
"$bin" mem "$tmp/e_dense/ref.fa" "$reads" 2> /dev/null | grep -v '^@' > "$tmp/e_dense.sam" \
    || fail "case E: alignment against the re-sa'd index failed"
...
grep -qE "SA sample rate 1/8 \(shift 3\)" "$tmp/f/inspect.log" \
    || {
        cat "$tmp/f/inspect.log" >&2
        fail "case F: inspection must report the current rate (1/8)"
    }
...
# --- Case H: permission preservation ---------------------------------------
# re-sa rewrites the index in place through a mkstemp temp (created 0600). It
# must publish with the destination's existing permission bits, not a hard-coded
# 0644, so resampling a deliberately private index does not silently widen it to
# world-readable. Portable mode read: GNU `stat -c` then BSD `stat -f`.
file_mode() {
    stat -c '%a' "$1" 2> /dev/null || stat -f '%Lp' "$1"
}
h_idx="$(build_index "$tmp/h")" # default rate = stride 8
chmod 0600 "$h_idx"
"$bin" re-sa -u 2 "$tmp/h/ref.fa" > "$tmp/h/resa.log" 2>&1 || {
    cat "$tmp/h/resa.log" >&2
    fail "case H: re-sa -u 2 failed"
}
h_mode="$(file_mode "$h_idx")"
[[ "$h_mode" == "600" ]] || fail "case H: re-sa widened index mode to $h_mode (expected 600)"

echo "PASS: re-sa produces a byte-identical index to \`index -u\` (densify + coarsen), is thread-deterministic, no-ops on the current rate, preserves alignments, reports the rate in inspection mode, preserves the index's file permissions, and rejects bad arguments"
